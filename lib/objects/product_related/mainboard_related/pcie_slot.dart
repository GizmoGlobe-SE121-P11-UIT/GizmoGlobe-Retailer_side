...
class PCIeSlot {
  ...
  PCIeSlot(
      ...
      required this.electricalSpeed,
      required this.gen,
      required this.quantity});

  factory PCIeSlot.fromJson(Map<String, dynamic> json) => PCIeSlot(
        physicalSize: (json['physicalSize'] is num)
            ? (json['physicalSize'] as num).toInt()
            : int.tryParse(json['physicalSize']?.toString() ?? '') ?? 0,
        electricalSpeed: (json['electricalSpeed'] is num)
            ? (json['electricalSpeed'] as num).toInt()
            : int.tryParse(json['electricalSpeed']?.toString() ?? '') ?? 0,
        gen: (json['gen'] is num)
            ? (json['gen'] as num).toInt()
            : int.tryParse(json['gen']?.toString() ?? '') ?? 0,
  ...
  Map<String, dynamic> toJson() => {
        ...
        'electricalSpeed': electricalSpeed,
        'gen': gen,
        'quantity': quantity,
      };
...
class PCIeSlotControllers {
  ...
  PCIeSlotControllers({
    ...
    String? quantity,
        ...
        electricalSpeedController =
            TextEditingController(text: electricalSpeed),
